...
para("En mars 2026, le paysage des outils d'intelligence artificielle evolue a une vitesse vertigineuse. En une seule semaine de mars 2026, plus de 12 modeles majeurs ont ete lances simultanement — GPT-5.4 d'OpenAI, Gemini 3.1 de Google, Claude Opus 4.6 d'Anthropic, Qwen 3.5 d'Alibaba, et bien d'autres. Des assistants conversationnels aux generateurs d'images, en passant par les outils de code, de video et de musique, l'IA est devenue un ecosysteme complet d'outils interconnectes qui transforment chaque aspect du travail creatif et professionnel.");
para("Le marche des outils IA est domine par quelques acteurs majeurs — OpenAI, Anthropic, Google, Meta — mais une multitude de startups et de projets open source (Qwen, DeepSeek, Mistral) viennent enrichir l'offre avec des solutions specialisees. Tous les modeles de pointe atteignent desormais 1 million de tokens de contexte et integrent le Computer Use (capacite a controler un ordinateur). La cle du succes n'est plus simplement de connaitre un seul outil, mais de savoir naviguer dans cet ecosysteme et choisir le bon outil pour chaque tache.");
...
para("La plupart des outils IA offrent un niveau gratuit permettant de decouvrir leurs fonctionnalites. ChatGPT Free (avec GPT-5.4 mini), Claude Free, et Gemini Free sont accessibles a tous. Les versions payantes (ChatGPT Plus a 20$/mois, ChatGPT Pro a 200$/mois, Claude Pro a 20$/mois) offrent les modeles les plus puissants (GPT-5.4 Pro, Claude Opus 4.6), des limites d'utilisation plus elevees, et des fonctionnalites avancees comme le Computer Use, la generation d'images, la navigation web, et l'analyse de documents.");
...
summaryBox(["Mars 2026 : 12+ modeles majeurs lances en une semaine, 1M tokens standard", "Categories : texte, image, code, video, audio, recherche, Computer Use", "Niveaux gratuits disponibles (GPT-5.4 mini, Claude Free, Gemini Free)", "Choisir le bon outil pour chaque tache est une competence cle"]);

// CH2
chapterTitle(2, "ChatGPT : Guide Complet");
doc.moveDown(2);
sectionTitle("2.1 Presentation de ChatGPT");
para("ChatGPT, developpe par OpenAI, est l'outil IA le plus populaire au monde avec plus de 400 millions d'utilisateurs actifs en 2026. Disponible via le web, des applications mobiles et une API, ChatGPT est un assistant conversationnel polyvalent capable de rediger du texte, repondre a des questions, analyser des documents, generer du code, creer des images (via DALL-E integre), controler un ordinateur (Computer Use), et bien plus encore.");
para("En mars 2026, ChatGPT est propulse par la famille GPT-5.4, sortie le 5 mars 2026. GPT-5.4 Standard est le modele generaliste polyvalent. GPT-5.4 Thinking est optimise pour le raisonnement avance (mathematiques, logique, code complexe). GPT-5.4 Pro offre la capacite maximale pour les taches les plus exigeantes. Tous disposent d'un contexte d'1,05 million de tokens — assez pour analyser un livre entier. GPT-5.4 mini et nano (sortis le 17 mars 2026) rendent cette puissance accessible gratuitement. Les hallucinations sont reduites de 33% par rapport a GPT-5.2.");
...
summaryBox(["ChatGPT est l'outil IA le plus utilise avec 400M+ d'utilisateurs", "Modeles GPT-5.4 : Standard, Thinking, Pro + mini/nano", "Contexte de 1,05 million de tokens, Computer Use natif", "33% moins d'hallucinations que la generation precedente", "Version Plus a 20$/mois, Pro pour les fonctionnalites avancees"]);

// CH3
chapterTitle(3, "Claude par Anthropic");
doc.moveDown(2);
sectionTitle("3.1 Presentation de Claude");
para("Claude est l'assistant IA developpe par Anthropic, une entreprise fondee par d'anciens chercheurs d'OpenAI. En mars 2026, Claude s'est impose comme l'un des meilleurs assistants IA du marche, reconnu pour sa precision, son honnetete intellectuelle (il admet ce qu'il ne sait pas), sa capacite a suivre des instructions complexes, et ses performances exceptionnelles en redaction, en programmation et en analyse financiere.");
para("Claude Opus 4.6, lance le 5 fevrier 2026, est le modele phare d'Anthropic. Avec une fenetre de contexte d'1 million de tokens (assez pour des bibliotheques entieres de documents), un horizon de completion de taches de 14,5 heures (le plus long du marche), et le meilleur score sur le benchmark Finance Agent, c'est un outil de travail professionnel sans equivalent. Claude Sonnet 4.6 offre le meilleur equilibre performance/vitesse avec des ameliorations majeures en code, computer use, raisonnement long-contexte et planification d'agents. Haiku 4.5 reste le modele rapide et economique.");
...
para("Grace a sa fenetre de contexte d'1 million de tokens, Claude peut analyser des documents entiers — livres complets, rapports annuels, contrats juridiques, codebases entieres — et en extraire les informations pertinentes. Cette capacite est inegalee pour les taches de recherche, de synthese, et d'analyse documentaire. Le Computer Use de Claude atteint 72,5% sur OSWorld, lui permettant de naviguer dans des environnements informatiques reels.");
checkNewPage();
...
summaryBox(["Claude se distingue par sa precision et son honnetete", "Modeles : Opus 4.6 (le plus puissant), Sonnet 4.6 (equilibre), Haiku 4.5 (rapide)", "Fenetre de contexte d'1 million de tokens", "Computer Use a 72,5% sur OSWorld, horizon de tache de 14,5h", "Claude Code integre l'IA directement dans le terminal de developpement"]);
...
para("En mars 2026, Gemini 3.1 Pro est le modele Pro le plus avance de Google, lance en fevrier 2026. Avec un contexte d'1 million de tokens, un score de 77,1% sur ARC-AGI-2 (un benchmark de raisonnement general), et un raisonnement multimodal sur texte, images, audio, video et code, c'est un concurrent serieux pour GPT-5.4 et Claude 4.6. Gemini est integre dans l'ensemble de l'ecosysteme Google : Search (AI Overviews), Gmail, Google Docs, Sheets, Slides, et Android.");
sectionTitle("4.2 Gemini vs ChatGPT vs Claude");
para("Chaque assistant a ses forces. En mars 2026, les trois geants ont atteint la parite sur le contexte (1M+ tokens) et le Computer Use. Gemini 3.1 excelle dans l'integration avec les services Google et le raisonnement multimodal natif. GPT-5.4 est le plus polyvalent avec le plus grand ecosysteme (GPTs, plugins, Computer Use natif). Claude 4.6 est le plus precis et le plus fiable pour la redaction, l'analyse technique, la finance, et les taches longues (14,5h d'autonomie).");
para("En pratique, les professionnels avises utilisent plusieurs outils selon la tache : Gemini pour la recherche et l'integration Google Workspace, Claude pour la redaction, le code et l'analyse approfondie, ChatGPT pour sa polyvalence et son ecosysteme. La maitrise de plusieurs outils est un avantage competitif majeur.");
summaryBox(["Gemini 3.1 Pro : nativement multimodal, 77,1% sur ARC-AGI-2", "Integration profonde dans l'ecosysteme Google (Search, Workspace, Android)", "Contexte d'1 million de tokens, comme GPT-5.4 et Claude 4.6", "Complementaire a ChatGPT et Claude selon les cas d'usage"]);

// CH5
chapterTitle(5, "Midjourney : Creation d'Images");
doc.moveDown(2);
sectionTitle("5.1 Qu'est-ce que Midjourney ?");
para("Midjourney est un outil de generation d'images par IA qui transforme des descriptions textuelles (prompts) en images de haute qualite. Repute pour son esthetique artistique exceptionnelle, Midjourney est devenu l'outil de reference pour les graphistes, illustrateurs, marketeurs, et createurs de contenu. La V7 est le modele par defaut depuis juin 2025, et la V8 Alpha a ete lancee le 17 mars 2026 avec des images HD en 2K natif. Midjourney est accessible via Discord et son site web dedie.");
sectionTitle("5.2 Ecrire des Prompts Efficaces");
para("La qualite de l'image generee depend directement de la qualite du prompt. Un bon prompt Midjourney comprend : le sujet principal, le style artistique souhaite, l'eclairage, l'angle de vue, les couleurs dominantes, et eventuellement des references a des artistes ou des techniques. Par exemple : « portrait of a young woman, oil painting style, dramatic lighting, rich colors, Rembrandt-inspired --ar 3:4 --v 7 ».");
subSection("Les Parametres Essentiels");
para("--ar (aspect ratio) : definit le rapport largeur/hauteur (16:9, 1:1, 3:4, etc.). --v (version) : selectionne la version du modele (7 par defaut, 8 en alpha). --s (stylize) : controle l'intensite artistique (0-1000). --c (chaos) : ajoute de la variabilite. --q (quality) : ajuste la qualite de rendu. --no : exclut des elements. --style raw : reduit l'interpretation artistique. --hd : images HD 2K (V8 Alpha). --oref (omni-reference) : coherence de personnages et objets entre les generations.");
checkNewPage();
sectionTitle("5.3 Techniques Avancees");
para("La V7 a introduit les profils de personnalisation — le modele apprend vos preferences esthetiques au fil du temps. Le Vary Region permet de modifier une partie specifique d'une image generee. Le Zoom Out etend l'image au-dela de ses bordures. Le Pan permet de deplacer le cadrage. Le Blend melange plusieurs images de reference en une seule. L'omni-reference (--oref) permet de maintenir la coherence d'un personnage ou objet entre plusieurs generations. Le mode Draft genere a 10x la vitesse normale pour moitie prix.");
...
summaryBox(["Midjourney V7 (defaut) + V8 Alpha (mars 2026) avec images HD 2K", "Un bon prompt inclut : sujet, style, eclairage, angle, couleurs", "Parametres cles : --ar, --v, --s, --c, --oref, --hd (V8)", "Profils de personnalisation, Draft mode 10x plus rapide, omni-reference"]);
...
para("Sora 2, la derniere version du generateur video d'OpenAI, a remplace Sora 1 (deprecie le 13 mars 2026). Sora 2 genere des videos avec dialogues synchronises et effets sonores, une physique nettement plus realiste (rebonds de ballon, dynamique des fluides, mouvements de gymnastique), et un controle accru. Une application sociale iOS dediee permet de creer, remixer les generations d'autres utilisateurs, et integrer des « personnages » (vous ou vos amis) dans les videos.");
para("Sora 2 est en cours d'integration directement dans ChatGPT, ce qui permettra a des centaines de millions d'utilisateurs de generer des videos depuis leur interface habituelle. Les applications vont de la creation de contenu pour les reseaux sociaux a la pre-visualisation pour le cinema, en passant par la publicite, l'education, et le prototypage de concepts visuels.");
...
summaryBox(["Sora 2 : videos avec dialogues synchronises, effets sonores et physique realiste", "Application sociale iOS + integration prevue dans ChatGPT", "Sora 1 deprecie le 13 mars 2026, Sora 2 est le nouveau standard", "Alternatives : Runway Gen-3, Pika Labs, Kling, Minimax"]);
...
para("Pour la redaction : Claude 4.6 > GPT-5.4 > Gemini 3.1. Pour le code : Claude Code / Cursor > Copilot > GPT-5.4. Pour les images artistiques : Midjourney V7/V8 > Stable Diffusion > DALL-E. Pour la recherche : Perplexity > Gemini > ChatGPT. Pour l'integration Google : Gemini. Pour les taches longues et autonomes : Claude 4.6 (14,5h d'autonomie). Pour le Computer Use : GPT-5.4 et Claude 4.6. Ces classements evoluent rapidement — GPT-5.4 et Claude 4.6 sont au coude a coude sur la majorite des benchmarks.");
